...
fetch_native_hash() {
    ...
    local url="${GITHUB_RELEASE_BASE}/v${version}/opencode-${platform}.${extension}"
...
update_to_version() {
    ...
    log_info "  Building opencode (native)..."
    if ! nix build .#opencode > /dev/null 2>&1; then
...
print_usage() {
    ...
    echo "  $0 --version 1.1.0    # Update to specific version"
}
